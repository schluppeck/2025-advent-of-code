...
#
# exploring all the paths may be possible
# it's not 2^142 because the choices are more sparse.
#
# ABANDONED - see notes in README.md
# there is a working solution (but by ChatGPT!) 
# in solutionBchatGPT.jl = worth a study...
...
function simulateBeam(M)
    ...
    # initialise the new timelines collection!
    nTimeLineHops = [1 for i in 1:nRows] # per column?

    # from the 3rd row onwards, simulate beam travel
    # by looking above and placing beams...
    nHits = 0 # per timeline 
    nTravels = 0 # per timeline?
    ...
    return N, nHits, nTravels, allHits  
end

N, nHits, nTravels, allHits = simulateBeam(data)
